...
assert.match(worker, /const BUILD = "1\.4\.18-diagnose6"/);
assert.match(play, /--card-gap:14px/);
assert.match(play, /\.card\{margin-bottom:var\(--card-gap\)/);
assert.match(tvSw, /hitster-tv-v1\.4\.18-diagnose6/);
console.log('1.4.18-diagnose6: Cloudflare-Build und einheitlicher Browser-Kartenabstand bestanden');
